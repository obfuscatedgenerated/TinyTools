...
ap = argparse.ArgumentParser(
    description="Manage TinyTools and run a tool directly to bypass PATH conflicts."
)

ap.add_argument("action", type=str, help="the action you want to call (list, run)")
...
if args.action == "list":
    ...
    print(
        Fore.YELLOW
        + "Deprecation Warning: v1 syntax is being phased out! Use tt run {tool} instead of tt {tool}"
    )
    print(Style.RESET_ALL)
    os.system(
        os.path.join(asyncio.run(get_PATH_tt()), args.action)
        + " "
        + " ".join(args.arguments)
    )
